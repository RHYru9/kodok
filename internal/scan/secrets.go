		...
		for _, pattern := range s.patterns {
			...
			for _, match := range matches {
				if len(match) > 0 {
					secretValue := match[0]
					if len(match) > 1 {
						secretValue = match[1] // Use captured group
					}
...
func (s *SecretScanner) validateSecret(secretType, value string) bool {
	falsePositives := []string{
		...
		"your_", "fake_", "dummy_", "123456", "password", "admin",
		"guest", "root", "letmein", "welcome", "qwerty", "default",
		"iloveyou", "login", "user", "pass", "secret",
	}
